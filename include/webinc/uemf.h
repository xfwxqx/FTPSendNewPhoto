...
#define B_ARGS			file, line
#endif /* B_L */
extern void* balloct(B_ARGS_DEC,int num);
extern void* brealloct(B_ARGS_DEC,void* p, int num);
#define balloc balloct
#define bfree(B_ARGS, p) free(p)
#define bfreeSafe(B_ARGS, p) \
	if (p) { free(p); } else
#define brealloc	brealloct
...
extern  void	valueInteger(long value,value_t *vt);
extern 	void 	valueString(char_t *value, int flags,value_t *vt);
extern 	void 	valueErrmsg(char_t *value,value_t *vt);
